...
const MapComponent = () => {
  ...
  useEffect(() => {
    ...
    if (!mapInstance.current) {
      mapInstance.current = L.map(mapRef.current).setView([31.2414354, 29.9672746], 18);
    ...
    const drawMarkersAndPolyline = (data) => {
      ...
      // تحويل الإحداثيات مع ضبط الدقة
      const latlngs = data.map(loc => {
        // تأكد من صحة ترتيب الإحداثيات (قد تحتاج لعكسها إذا كانت مقلوبة)
      ...
      latlngs.forEach((latlng, index) => {
        ...
        .addTo(mapInstance.current)
        .bindPopup(`النقطة ${index + 1}<br>${latlng[0].toFixed(6)}, ${latlng[1].toFixed(6)}`);
      ...
      // ضبط حدود الخريطة مع هامش مناسب
