...
const Rooms = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get("http://147.79.101.225:8888/admin/room", {
                    headers: { Authorization: `Bearer ${token}` },
                });
                setAirline(response.data.rooms);
            } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post(
                "http://147.79.101.225:8888/admin/room",
                formData,
                { headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" } }
            );

            setAirline([...airlines, response.data.Room]);
    ...
    const handleConfirmDelete = async () => {
        try {
            await axios.delete(`http://147.79.101.225:8888/admin/room/${airlineToDelete}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
