...
const BookingTravelRequests = () => {
    ...
    const fetchRequests = async () => {
      try {
        const response = await axios.get(
          "http://147.93.53.128:8888/admin/request/travel",
          {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://147.93.53.128:8888/admin/request/travel/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <div className="travel-requests-wrapper">
      <h1 className="page-title">طلبات حجز الطيران</h1>
